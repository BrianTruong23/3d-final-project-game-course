...
public class AutoSetup
{
    ...
    public static void Run()
    {
        ...
        // Clear old enemies so we can spawn them in the new correct locations
        ...
        // We will spawn 6 enemies total (3 Huskies, 3 Wolves) near the player start (789, 0, 582)
        string[] enemyPaths = { "Assets/Enemies/Husky.obj", "Assets/Enemies/Husky.obj", "Assets/Enemies/Husky.obj", 
                                "Assets/Enemies/Wolf.obj", "Assets/Enemies/Wolf.obj", "Assets/Enemies/Wolf.obj" };
        foreach (var path in enemyPaths)
        {
            Vector3 pos = new Vector3(789f + Random.Range(-12f, 12f), 0, 582f + Random.Range(-12f, 12f));
            pos.y = GetGroundHeight(pos);

            SpawnPrefab(path, enemiesParent.transform, (go) => {
                var bc = go.GetComponent<BoxCollider>();
                if (bc == null) bc = go.AddComponent<BoxCollider>();
                bc.size = new Vector3(4, 4, 4);
                bc.center = new Vector3(0, 2f, 0);
                bc.isTrigger = false; // Solid collision

                var rb = go.GetComponent<Rigidbody>();
                if (rb == null) rb = go.AddComponent<Rigidbody>();
                rb.isKinematic = false; // Affected by gravity and collision
                rb.constraints = RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationZ;
                
                var ed = go.GetComponent<EnemyDamage>();
                if (ed == null) ed = go.AddComponent<EnemyDamage>();
